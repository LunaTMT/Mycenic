...
export default function CustomerOrders() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      console.log("[CustomerOrders] useEffect triggered. user:", user);

      if (!user?.id) {
        console.warn("[CustomerOrders] No user id, skipping fetch.");
        return;
      }

      setLoading(true);
      console.log("[CustomerOrders] Fetching orders for user:", user.id);
      ...
      try {
        ...
        console.log("[CustomerOrders] Response received:", res);

        setOrders(res.data.orders || []);
        console.log("[CustomerOrders] Orders set:", res.data.orders || []);
      } catch (error) {
        console.error("[CustomerOrders] Failed to fetch orders:", error);
      } finally {
        setLoading(false);
