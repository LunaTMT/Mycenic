  ...
  export default function ReturnInstructions(props: ReturnInstructionsProps) {
    ...
    const createPaymentIntent = async (amount: number) => {
      console.log("🔄 Starting createPaymentIntent with amount:", amount);

      setIsCreatingIntent(true);
      console.log("📌 isCreatingIntent set to true");

      try {
        const roundedAmount = Math.round(amount * 100);
        console.log("💰 Sending amount (in cents):", roundedAmount);
        ...
        if (response.ok && data.clientSecret) {
          ...
          console.error("❌ Failed to create payment intent. Server responded with:", data);
        }
      } catch (error) {
        console.error("🔥 Error occurred while creating payment intent:", error);
      } finally {
        setIsCreatingIntent(false);
        console.log("🧹 isCreatingIntent set to false");
      }
    };
    ...
    const handleShippingOptionChange = (option: string) => {
      console.log('🏷️  Parent received onChange with:', option);
      setSelectedShippingOption(option);
    };

    useEffect(() => {
      console.log('📊 selectedShippingOption is now:', selectedShippingOption);
